...
TM_USB_HIDDEVICE_Status_t TM_USB_HIDDEVICE_GamepadSend(TM_USB_HIDDEVICE_Gamepad_Number_t gamepad_id, TM_USB_HIDDEVICE_Gamepad_t* Gamepad_Data) {
	...
	// DS4 R3 L3 Option Share R2 L2 R1 L1
	buff[6] = 0;
...
TM_USB_HIDDEVICE_Status_t TM_USB_HIDEVICE_DualShock4_StructInit(TM_USB_HIDDEVICE_DualShock4_t* DualShock_Data) {
	...
	DualShock_Data->battery = 0x0B; //100% power-0x0B
...
//	}
	...
	buff[0] = gamepad_id; /* gamepad id */
		...
		/* Left stick axis X, byte 1 */
	buff[1] = DS4_Data->LeftXAxis;
		/* Buttons pressed, byte 1 */
	buff[2] = DS4_Data->LeftYAxis;
		/* Buttons pressed, byte 1 */
	buff[3] = DS4_Data->RightXAxis;
		/* Buttons pressed, byte 1 */
	...
	buff[5] |= DS4_Data->circle 	<< 6;	/* Bit 6 */
	buff[5] |= DS4_Data->Triangle 	<< 7;	/* Bit 7 */
	
	/* Buttons pressed, byte 2 */
	// DS4 R3 L3 Option Share R2 L2 R1 L1
	buff[6] = 0;
	buff[6] |= DS4_Data->L1 	<< 0;	/* Bit 0 */
	buff[6] |= DS4_Data->R1 	<< 1;	/* Bit 1 */
	buff[6] |= DS4_Data->L2 	<< 2;	/* Bit 2 */
	buff[6] |= DS4_Data->R2 	<< 3;	/* Bit 3 */
	buff[6] |= DS4_Data->start 	<< 4;	/* Bit 4 */
	buff[6] |= DS4_Data->option << 5;	/* Bit 5 */
	buff[6] |= DS4_Data->L3 	<< 6;	/* Bit 6 */
	buff[6] |= DS4_Data->R3 	<< 7;	/* Bit 7 */
